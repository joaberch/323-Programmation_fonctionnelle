...
int totalPeach = (from p in products where p.ProductName == "Pêches" select p).Count();
Product mostWatermelon = (from p in products where p.ProductName == "Pastèques" orderby p.Quantity descending select p).Take(1).Single();
...
//List<Product> waterMelon = (List<Product>)(from p in products where p.ProductName == "Pastèques" select p);
//List<Product> pasteque = (List<Product>)(from p in products where p.ProductName == "Melons" select p);

//List<Product> pastequeMelon = (List<Product>)( from w in waterMelon join p in pasteque on w.Producer equals p.Producer select new
//{
//    p.Producer,
//    p.Quantity,
//    w.Quantity
